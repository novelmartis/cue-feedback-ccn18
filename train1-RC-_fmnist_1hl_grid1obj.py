...
train_step = tf.train.GradientDescentOptimizer(1e-1).minimize(cross_entropy) # 1e-1 for 3072, 0.7e-1 for 32, 5e-2 for 16, 1e-1 for 8

tf.summary.scalar('cross_entropy_none_1obj_8', cross_entropy)
tf.summary.scalar('accuracy_none_1obj_8', accuracy)
...
with tf.Session() as sess:
  ...
  #saver.restore(sess,"./models_new/P3_fmnist_1hl_none_grid1obj_32.ckpt")
  ...
  for step in range(max_steps):
    ...
    batch_xs, batch_ys = gridify(batch_xs,batch_ys,1)
    batch_xs_r, batch_ys_r = random_pattern_gen(16)
    if (step % 100) == 0:
      print('Train: ',step, sess.run(accuracy, feed_dict={x: batch_xs, y_: batch_ys, keep_prob: 1.0}))
      summary = sess.run(merged, feed_dict={x: batch_xs, y_: batch_ys, keep_prob: 1.0})
      train_writer.add_summary(summary, step)
      print('Random_t: ',step, sess.run(accuracy, feed_dict={x: batch_xs_r, y_: batch_ys_r, keep_prob: 1.0}))
    if (step % 500) == 0:
      ...
      batch_xsv, batch_ysv = gridify(batch_xsv,batch_ysv,1)
      print('Val: ',step, sess.run(accuracy, feed_dict={x: batch_xsv, y_: batch_ysv, keep_prob: 1.0}))
      summary = sess.run(merged, feed_dict={x: batch_xsv, y_: batch_ysv, keep_prob: 1.0})
      val_writer.add_summary(summary, step)
      print('Random_v: ',step, sess.run(accuracy, feed_dict={x: batch_xs_v, y_: batch_ys_v, keep_prob: 1.0}))
  ...
  batch_ys = batch_mod_test(batch_ys)
  batch_xs, batch_ys = gridify(batch_xs,batch_ys,1)
  print(max_steps, sess.run(accuracy, feed_dict={x: batch_xs, y_: batch_ys, keep_prob: 1.0}))

  saver = tf.train.Saver({"W_fc1": W_fc1, "b_fc1": b_fc1, "W_fc2": W_fc2, "b_fc2": b_fc2},write_version=tf.train.SaverDef.V1)
  if data_flag == 1:
    save_path = saver.save(sess, "./models_new/P3_fmnist_1hl_none_grid1obj_8.ckpt")
  elif data_flag == 2:
    save_path = saver.save(sess, "./models_new/P3_mnist_1hl_none_grid1obj_8.ckpt")
