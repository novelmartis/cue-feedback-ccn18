## Initialising tensorflow and other necessary functions ----------------------------------------------
...
## Initialising the neural network parameters ----------------------------------------------
...
cond_h = 7 # 1 for ffonly, 7 for fb on ff
max_blur = 4.
...
if cond_h == 1: # 1.3 for rest, 0.5 for 8
  train_step_ff = tf.train.GradientDescentOptimizer(0.5).minimize(cross_entropy_ff,var_list=[W_ff_h_f,W_ff_h_z,b_ff_h,W_ff,b_ff])
  train_step_ff_fb = tf.train.GradientDescentOptimizer(0.5).minimize(cross_entropy_ff_fb,var_list=[fb_W_ff_h_f,fb_W_ff_h_z,fb_b_ff_h,fb_W_ff,fb_b_ff])
  tf.summary.scalar('cross_entropy_none_1obj_fb_ff_fb_nobjblur_ffonly_8', cross_entropy_ff_fb)
  tf.summary.scalar('accuracy_none_1obj_fb_ff_fb_nobjblur_ffonly_8', accuracy_ff_fb)
  merged = tf.summary.merge_all()
if cond_h == 7: # 1 for rest, 0.3 for 8
  train_step_ff_fb = tf.train.GradientDescentOptimizer(0.3).minimize(cross_entropy_ff_fb-cross_entropy_ff_1,var_list=[W_gain_fc1,W_bias_fc1])
  train_step_ff_fb1 = tf.train.GradientDescentOptimizer(0.3).minimize(cross_entropy_ff_fb,var_list=[W_gain_fc1,W_bias_fc1])
  tf.summary.scalar('across_entropy_none_1obj_fb_ff_fb_1objblur4obj_fbonff_fp_tog_opt_8', cross_entropy_ff_fb-cross_entropy_ff_1)
  tf.summary.scalar('aaccuracy_none_1obj_fb_ff_fb_1objblur4obj_fbonff_fp_tog_opt_8', accuracy_ff_fb-accuracy_ff_1)
  merged = tf.summary.merge_all()
...
with tf.Session() as sess:
  ...
  train_writer = tf.summary.FileWriter('train_2',sess.graph)
  val_writer = tf.summary.FileWriter('val_2')

  #sess.run(tf.global_variables_initializer())
  if cond_h == 1: # ffonly
    saver.restore(sess,"./models_new/P3_fmnist_1hl_none_grid1obj_8.ckpt")
    sess.run(tf.initialize_variables(set(tf.all_variables()) - temp))
    max_steps = 350001
  if cond_h == 7: #fbonff
    saver.restore(sess,"./models_new/P3_fmnist_1hl_none_grid1obj_fbff_nofp_ff_nofp_bgjoint_nobjblur_ffonly_8.ckpt")
    sess.run(tf.initialize_variables(set(tf.all_variables()) - temp))
    max_steps = 350001
  ...
  if cond_h == 1:
    save_path = saver.save(sess, "./models_new/P3_fmnist_1hl_none_grid1obj_fbff_nofp_ff_nofp_bgjoint_nobjblur_ffonly_8.ckpt")
    print("Model saved in file: %s" % save_path)
  if cond_h == 7:
    save_path = saver.save(sess, "./models_new/P3_fmnist_1hl_none_grid1obj_fbff_nofp_ff_nofp_bgjoint_nobjblur_fbonff_tog_opt_8.ckpt")
    print("Model saved in file: %s" % save_path)